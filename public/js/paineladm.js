...
function getCurrentShiftData() {
  ...
  console.log(
    `Obtendo dados para data: ${selectedDate}, turno: ${activeShift}`,
...
function changeShift(newShift) {
  if (
    ...
    ["manhã", "tarde", "noite"].includes(newShift)
  ) {
    console.log(
      `Mudando turno de ${activeShift} para ${newShift} na data ${selectedDate}`,
    );
    ...
    const shiftCapitalized =
      newShift.charAt(0).toUpperCase() + newShift.slice(1);
    const [year, month, day] = selectedDate.split("-");
    const formattedDate = `${day}/${month}/${year}`;
    showNotification(
      `Visualizando turno da ${shiftCapitalized} - ${formattedDate}`,
      "info",
    );
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      const convertedData = data.map((row, index) => {
        ...
        const uniqueId =
          registro ||
          `import_${Date.now()}_${index}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      const validData = convertedData.filter((item) => {
        ...
        if (isValidRoom) {
          ...
          if (item.disciplina)
            item.disciplina = item.disciplina.toString().trim();
          if (item.professorName)
            item.professorName = item.professorName.toString().trim();
          if (item.turmaNumber)
            item.turmaNumber = item.turmaNumber.toString().trim();
      ...
      if (validData.length === 0) {
        throw new Error(
          "Nenhum registro válido encontrado no arquivo. Verifique o formato dos dados.",
        );
      }
      ...
      console.log(
        `Dados importados e ordenados com sucesso para o turno ${selectedShift}. Total de registros:`,
        sortedData.length,
      );
      ...
      if (typeof saveDataToFirebase === "function") {
        ...
        saveDataToFirebase(selectedDate, selectedShift, sortedData)
          .then(() => {
            console.log(" Dados importados salvos no Firebase com sucesso!");
          })
          .catch((error) => {
            console.error(
              " Erro ao salvar dados importados no Firebase:",
              error,
            );
          });
      }
...
function readFileData(file) {
  return new Promise((resolve, reject) => {
    ...
    console.log(
      `Tipo detectado: ${isCSVType ? "CSV/Texto" : isExcelType ? "Excel" : "Desconhecido"}`,
    );
    ...
    reader.onload = function (e) {
      try {
        ...
        if (isCSVType) {
          ...
          try {
            ...
            try {
              ...
              console.log(
                "Tentativa ISO-8859-1 falhou, tentando Windows-1252...",
              );
          ...
          if (
            ...
            throw new Error(
              `Arquivo ${fileExt.toUpperCase()} inválido ou vazio`,
            );
          }
        } else if (isExcelType) {
          ...
          if (
            ...
            throw new Error(
              `Arquivo Excel (${fileExt}) inválido ou sem planilhas`,
            );
          }
        } else {
        ...
        console.log(
          "Dados brutos lidos do arquivo (com células mescladas):",
          allData,
        );
        ...
        if (firstSheet["!merges"]) {
          ...
          firstSheet["!merges"].forEach((merge) => {
            ...
            const firstCellValue = firstSheet[firstCellRef]
              ? firstSheet[firstCellRef].v
              : "";

            if (firstCellValue) {
              console.log(
                `� Propagando valor "${firstCellValue}" da célula mesclada ${firstCellRef}`,
              );
          ...
          console.log(
            "Dados após processar células mescladas:",
            allData.slice(0, 10),
          );
        }
        ...
        for (let i = 0; i < allData.length; i++) {
          ...
          const hasHeaderTerms = [
            ...
            "MATERIA",
            "MATÉRIA",
          ].some((term) => rowStr.includes(term));
        ...
        if (headerRowIndex === -1) {
          throw new Error(
            "Não foi possível detectar automaticamente o cabeçalho do arquivo. Verifique se o arquivo contém colunas como SALA, CURSO, PROFESSOR, etc.",
          );
        }
        ...
        console.log(
          `Processando ${dataRows.length} linhas de dados após o cabeçalho`,
        );
        console.log(
          "Primeiras 3 linhas de dados brutos:",
          dataRows.slice(0, 3),
        );
        ...
        const mappedData = dataRows
          .filter((row) => {
            ...
            if (!hasContent && dataRows.indexOf(row) < 5) {
              console.log(
                ` Linha ${dataRows.indexOf(row) + 1} filtrada (sem conteúdo):`,
                row,
              );
        ...
        console.log(
          " Processando dados para preencher campos de células mescladas...",
        );
          ...
          Object.keys(currentRow).forEach((key) => {
            if (
              ...
              if (previousRow[key] && previousRow[key] !== "") {
                ...
                if (i < 3)
                  console.log(
                    `� Preenchendo campo vazio "${key}" com valor "${previousRow[key]}" da linha anterior`,
                  );
              }
            }
          });
        ...
        const formattedData = mappedData
          .map((obj, index) => {
            ...
            const findColumn = (patterns, excludePatterns = []) => {
              ...
              if (index === 0) {
                console.log(
                  ` [Registro ${index + 1}] Procurando coluna para padrões: ${patterns.join(", ")}`,
                );
                console.log(
                  ` [Registro ${index + 1}] Excluir padrões: ${excludePatterns.join(", ")}`,
                );
                console.log(` [Registro ${index + 1}] Objeto disponível:`, obj);
                console.log(
                  ` [Registro ${index + 1}] Chaves disponíveis:`,
                  Object.keys(obj),
                );
              }
              ...
              for (let pattern of patterns) {
                for (let key in obj) {
                  ...
                  if (shouldExclude) {
                    if (index === 0)
                      console.log(
                        `   Excluindo coluna "${key}" (contém: ${excludePatterns.join(", ")})`,
                      );
                    continue;
                  }
                  ...
                  if (keyUpper === patternUpper) {
                    ...
                    if (value && String(value).trim() !== "") {
                      if (index === 0)
                        console.log(
                          `   Encontrado por correspondência exata: "${key}" = "${value}"`,
                        );
                  ...
                  if (keyUpper.includes(patternUpper)) {
                    ...
                    if (value && String(value).trim() !== "") {
                      if (index === 0)
                        console.log(
                          `   Encontrado por inclusão: "${key}" = "${value}"`,
                        );
                      return String(value).trim();
                    }
                  }
            ...
            if (!disciplinaFinal) {
              if (index === 0) {
                console.log(
                  " Procurando disciplina manualmente - TODAS as colunas disponíveis:",
                );
                Object.keys(obj).forEach((key, idx) => {
                ...
                if (value && String(value).trim() !== "") {
                  ...
                  if (!knownFields.includes(valueTrim)) {
                    ...
                    if (
                      ...
                      if (index === 0)
                        console.log(
                          `� Disciplina encontrada por eliminação na coluna "${key}": "${disciplinaFinal}"`,
                        );
                      break;
                    }
                  }
                ...
                for (let i = 0; i < allKeys.length; i++) {
                  ...
                  if (value && String(value).trim() !== "") {
                    ...
                    if (
                      ...
                      if (
                        !valueTrim.match(/^(G\d+|\d+|SALA|LAB)$/i) &&
                        valueTrim.length > 3
                      ) {
                        disciplinaFinal = valueTrim;
                        if (index === 0)
                          console.log(
                            `� Disciplina encontrada por tentativa na coluna "${key}": "${disciplinaFinal}"`,
                          );
                        break;
                      }
                    }
            ...
            if (index < 3) {
              ...
              if (!disciplinaFinal) {
                console.log(
                  " Debug disciplina vazia - verificando todas as colunas:",
                );
                Object.keys(obj).forEach((key, idx) => {
                  const keyUpper = key.toUpperCase();
                  const isDisciplinaCol =
                    keyUpper.includes("DISCIPLINA") ||
                    keyUpper.includes("MATERIA") ||
                    keyUpper.includes("MATÉRIA");
                  console.log(
                    `  Coluna ${idx} "${key}": "${obj[key]}" ${isDisciplinaCol ? "← POSSÍVEL DISCIPLINA" : ""}`,
                  );
                });
            ...
            if (index < 5) {
              console.log(` Debug Sala ${index + 1}:`, {
                ...
                salaDash: sala === "---",
                salaRegex: sala
                  ? /^sala(s)?$/i.test(String(sala).trim())
                  : false,
            ...
            if (salaInvalida) {
              if (index < 5)
                console.log(
                  ` Registro ${index + 1} rejeitado por sala inválida: "${sala}" (string: "${salaStr}")`,
                );
              return null;
            }
            ...
            if (index < 5)
              console.log(
                ` Registro ${index + 1} aprovado com sala: "${sala}" (string: "${salaStr}")`,
              );
            ...
            if (disciplinaFinal) {
              ...
              if (disciplinaFinal === curso) {
                console.warn(
                  ` Disciplina "${disciplinaFinal}" é igual ao curso, procurando disciplina real...`,
                );
                disciplinaFinal = "";
                ...
                for (let key in obj) {
                  ...
                  if (
                    ...
                    if (
                      ...
                      console.log(
                        ` Disciplina corrigida: "${disciplinaFinal}" (encontrada na coluna "${key}")`,
                      );
                      break;
                    }
                  }
              ...
              if (disciplinaFinal === professor) {
                console.warn(
                  ` Disciplina "${disciplinaFinal}" é igual ao professor, procurando disciplina real...`,
                );
                disciplinaFinal = "";
                ...
                for (let key in obj) {
                  ...
                  if (
                    ...
                    if (
                      !valorTrim.match(/^(SALA|A\d+|B\d+|C\d+|\d+)$/i) &&
                      valorTrim.length > 2
                    ) {
                      disciplinaFinal = valorTrim;
                      console.log(
            ...
            return [sala, curso, turma, professor, disciplinaFinal, registro];
          })
          .filter((row) => row !== null);
        ...
        console.log(
          `- Registros rejeitados: ${mappedData.length - formattedData.length}`,
        );
...
Verifique se há dados válidos após a linha de cabeçalho.`);
        ...
        console.log(
          "Primeiros 3 registros formatados:",
        ...
        reject(
          new Error(
            "Erro ao processar o arquivo. Verifique se o formato está correto e se há dados válidos.",
          ),
        );
      }
    };
...
function loadSavedData() {
  ...
  if (newFormatData) {
    try {
      ...
      for (let date in dataByDateAndShift) {
        for (let turno in dataByDateAndShift[date]) {
          if (Array.isArray(dataByDateAndShift[date][turno])) {
            dataByDateAndShift[date][turno] = dataByDateAndShift[date][
              turno
            ].map((item) => {
      ...
      console.log(
        "Dados carregados e ordenados no novo formato:",
        dataByDateAndShift,
      );
      updateTable();
      return;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (dateSelector) {
    ...
    dateSelector.addEventListener("change", function () {
      ...
      console.log(
        `Dados encontrados para ${selectedDate} no turno ${activeShift}:`,
        shiftData,
      );
        ...
        loadAllDataForDate(selectedDate).then(() => {
          ...
          for (let turno in dataByDateAndShift[selectedDate]) {
            if (Array.isArray(dataByDateAndShift[selectedDate][turno])) {
              dataByDateAndShift[selectedDate][turno] = dataByDateAndShift[
                selectedDate
              ][turno].sort((a, b) => {
                const professorA = (a.professorName || "").trim();
                const professorB = (b.professorName || "").trim();
      ...
      const totalRecords =
        (dateData["manhã"]?.length || 0) +
        (dateData["tarde"]?.length || 0) +
        (dateData["noite"]?.length || 0);

      showNotification(
        `Visualizando ${formattedDate} - ${shiftData.length} registros no turno (${totalRecords} total)`,
        "info",
      );
    });
  }
  ...
  if (
    confirmImportBtn &&
    !confirmImportBtn.hasAttribute("data-listener-added")
  ) {
    ...
    confirmImportBtn.addEventListener("click", async function () {
      ...
      const checkedInput = document.querySelector(
        'input[name="importShift"]:checked',
      );
      if (!checkedInput) {
  ...
  window.addEventListener("storage", function (e) {
    if (
      e.key === "allDateShiftData" ||
      e.key === "allShiftData" ||
      e.key === "dataUpdateTimestamp"
    ) {
      console.log(
        "[ADMIN] Detectada atualização de dados em outra aba/janela, chave:",
        e.key,
      );
      ...
      if (e.key === "allDateShiftData" && e.newValue) {
        try {
          ...
          console.log(
            "[ADMIN] Dados atualizados e ordenados:",
            dataByDateAndShift,
          );
...
function saveNewTeacher() {
  ...
  if (fats.length < 2) {
    showNotification(
      "O código FATS deve ter pelo menos 2 caracteres.",
      "warning",
    );
    return false;
  }
  ...
  try {
    ...
    const currentMapping = JSON.parse(
      localStorage.getItem("docentesCodprof") || "{}",
    );
    ...
    if (currentMapping[name]) {
      showNotification(
        `O professor "${name}" já está cadastrado no sistema.`,
        "warning",
      );
      return false;
    }
    ...
    for (const [existingName, existingFats] of Object.entries(currentMapping)) {
      if (existingFats === fats) {
        showNotification(
          `O FATS "${fats}" já está sendo usado pelo professor: ${existingName}.`,
          "warning",
        );
        return false;
      }
    ...
    if (typeof addOrUpdateTeacherInFirestore !== "function") {
      showNotification(
        "Erro: Função de cadastro no Firestore não está disponível. Contate o suporte.",
        "danger",
      );
      return false;
    }
    addOrUpdateTeacherInFirestore(name, fats)
      .then(() => {
        showNotification(
          `Professor '<strong>${name}</strong>' cadastrado(a) com sucesso!`,
          "success",
        );
        // Disparar evento customizado para notificar outras partes do sistema
        window.dispatchEvent(
          new CustomEvent("teacherAdded", {
      ...
      .catch((error) => {
        console.error("❌ Erro ao cadastrar professor no Firestore:", error);
        showNotification(
          "Erro ao cadastrar professor no Firestore. Tente novamente ou contate o suporte.",
          "danger",
        );
      });
    return true;
...
function initializeAll() {
  ...
  if (!storedTeachers) {
    ...
    try {
      ...
      console.log(
        " Mapeamento docentesCodprof carregado com sucesso:",
        Object.keys(parsed).length,
        "professores",
      );
    } catch (error) {
      console.warn(
        " Mapeamento docentesCodprof inválido, reinicializando...",
        error,
      );
      localStorage.setItem("docentesCodprof", JSON.stringify({}));
    }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const addButton = document.querySelector(
      'button[title="Adicionar Nova Chave"]',
    );
    ...
    if (addButton) {
      ...
      const alternativeButton = document.querySelector(
        '[data-action="add-teacher"], .btn-add-teacher, #addTeacherBtn',
      );
  ...
  function saveNewTeacher() {
    ...
    try {
      ...
      for (const [existingName, existingFast] of Object.entries(
        currentMapping,
      )) {
        if (existingFast === fats) {
          console.warn("⚠️ FATS já em uso:", { fats, existingName });
          showNotification(
            `O FATS <strong>"${fats}"</strong> já está sendo usado pelo professor: <strong>${existingName}</strong>.`,
            "warning",
          );
          return;
        }
...
function handleSaveButton(e) {
  ...
  cells.forEach((cell, index) => {
    ...
    if (input) {
      ...
      switch (index) {
        case 0:
          updatedData.room = newValue;
          break;
        case 1:
          updatedData.course = newValue;
          break;
        case 2:
          updatedData.turmaNumber = newValue;
          break;
        case 3:
          updatedData.professorName = newValue;
          break;
        case 4:
          updatedData.subject = newValue;
          break;
  ...
  if (rowId && Object.keys(updatedData).length > 0) {
    ...
    // Reativar listeners de realtime após edição
    if (
      typeof selectedDate !== "undefined" &&
      typeof syncDataRealtime === "function"
    ) {
      syncDataRealtime(selectedDate, "manhã");
      syncDataRealtime(selectedDate, "tarde");
      syncDataRealtime(selectedDate, "noite");
...
function updateSharedDataRecord(recordId, updatedFields) {
  try {
      ...
      for (const shift in dataByDateAndShift[date]) {
        ...
        if (recordIndex !== -1) {
          ...
          if (typeof saveDataToFirebase === "function") {
            ...
            console.log(
              " [ADMIN] Edição de registro - Dados antes de enviar ao Firebase:",
            );
            ...
            if (records && Array.isArray(records)) {
              console.log(
                " [ADMIN] Sincronizando atualização com Firebase...",
                {
                  date,
                  shift,
              ...
              saveDataToFirebase(date, shift, records)
                .then(() => {
                  console.log(
                    ` [ADMIN] Dados sincronizados no Firebase para ${date}/${shift}`,
                  );
                })
                .catch((error) => {
                  console.error(
                    " [ADMIN] Erro ao sincronizar no Firebase:",
                    error,
                  );
                });
            } else {
              console.error(
                " [ADMIN] Dados inválidos - records não é um array:",
                records,
              );
            }
          }
        ...
        try {
          ...
          for (const shift in allShiftData) {
            ...
            if (Array.isArray(shiftRecords)) {
              const recordIndex = shiftRecords.findIndex(
                (record) => record && record.id === recordId,
              );
              ...
              if (recordIndex !== -1) {
                ...
                console.log(
                  ` Registro atualizado na estrutura legacy:`,
                  shiftRecords[recordIndex],
                );
                recordFound = true;
    ...
    if (!recordFound) {
      console.warn(
        ` Registro com ID ${recordId} não encontrado em nenhuma estrutura`,
      );
    }
...
function showDeleteConfirmationModal(recordId, row) {
  ...
  modal
    ...
    .addEventListener("click", function () {
      this.innerHTML =
        '<i class="bi bi-hourglass-split me-1"></i> Excluindo...';
      this.disabled = true;
      ...
      if (deletionResult.success) {
        ...
        this.innerHTML =
          '<i class="bi bi-trash3-fill me-1"></i> Deletar Registro';
        this.disabled = false;
      }
    });
  ...
  try {
    ...
    for (const date in dataByDateAndShift) {
      for (const shift in dataByDateAndShift[date]) {
        ...
        localStorage.setItem(
          "allShiftData",
          JSON.stringify(dataByDateAndShift[date]),
        );
        ...
        if (typeof saveDataToFirebase === "function") {
          // DEBUG: Verificar dados antes de enviar ao Firebase
          console.log(
            " [ADMIN] Exclusão de registro - Dados antes de enviar ao Firebase:",
          );
          console.log(" [ADMIN] - date:", date);
          console.log(" [ADMIN] - shift:", shift);
          console.log(" [ADMIN] - records length:", records.length);
          ...
          if (records && Array.isArray(records)) {
            ...
            saveDataToFirebase(date, shift, records)
              .then(() => {
                console.log(
                  " [ADMIN] Exclusão sincronizada com Firebase com sucesso!",
                );
              })
              .catch((error) => {
                console.error(
                  " [ADMIN] Erro ao sincronizar exclusão no Firebase:",
                  error,
                );
              });
          } else {
    ...
    if (!recordFound) {
      ...
      if (allShiftDataStr) {
        ...
        for (const shift in allShiftData) {
          ...
          deletionDetails = {
            shift,
            deletedRecord: { ...shiftRecords[recordIndex] },
            source: "legacy",
          };
...
function showMensageConfirmationModal(nameTeacher, fats) {
  ...
  // Event listener para confirmação
  modal
    .querySelector("#confirmMessageBtn")
    .addEventListener("click", function () {
      bootstrapModal.hide();
    });
...
window.addEventListener("dataUpdated", function (event) {
  if (event.detail.type === "recordDeleted") {
    const row = document.querySelector(
      `tr[data-record-id="${event.detail.recordId}"]`,
    );
...
function renderTable() {
  ...
  const manualAllocationsContainer = document.getElementById(
    "manualAllocationsTableBody",
  );
  ...
  const shiftCapitalized =
    activeShift.charAt(0).toUpperCase() + activeShift.slice(1);
...
function isValidRecord(item) {
  ...
  const room = getFirstValidValue(item, [
    "room",
    "sala",
    "roomName",
    "classroom",
...
function normalizeRecord(item) {
  const normalized = {
    ...item,
    room:
      getFirstValidValue(item, ["room", "sala", "roomName", "classroom"]) || "",
    professorName:
      getFirstValidValue(item, ["professorName", "professor", "teacherName"]) ||
      "",
    course: getFirstValidValue(item, ["course", "curso"]) || "",
    subject:
      getFirstValidValue(item, ["subject", "disciplina", "materia"]) || "",
    turmaNumber: getFirstValidValue(item, ["turmaNumber", "turma"]) || "",
    withdrawalTime:
      getFirstValidValue(item, ["withdrawalTime", "horaRetirada"]) || "",
...
function getFirstValidValue(obj, fields) {
  return (
    fields.find((field) => obj[field] && obj[field].toString().trim()) &&
    obj[fields.find((field) => obj[field] && obj[field].toString().trim())]
  );
}

// Função de determinação do status (em uso, devolvida ou disponível)
function determineStatus(record) {
  const hasWithdrawal = getFirstValidValue(record, [
    "withdrawalTime",
    "horaRetirada",
  ]);
...
function generateId(record) {
  ...
  const prof = (
    getFirstValidValue(record, ["professorName", "professor"]) || "unknown"
  ).replace(/\s+/g, "");
...
function generateTableRow(record) {
  ...
  if (record.tipo === "manual_allocation") {
    console.log(" [DEBUG] Gerando linha para alocação manual:", {
      id: record.id,
      sala: record.sala,
      bloco: record.bloco,
...
function generateTeacherRow(nome, fats) {
  if (!nome || nome.trim() === "")
    return generateErrorRow("Nome do professor inválido");
...
function updateTeacherTable(teachersData = null) {
  ...
  try {
    ...
    tableContainer.innerHTML = generateTeacherTableHTML(
      teacherManagerState.teachers,
    );
    addTeacherEventListeners();
    return true;
  } catch (error) {
    console.error("Error updating teacher table:", error);
    tableContainer.innerHTML = generateErrorHTML(
      "Erro ao carregar tabela de professores",
    );
    return false;
  }
}
...
function startEditTeacher(teacherName) {
  ...
  if (!nameCell || !fatsCell || !editDeleteGroup || !saveCancelGroup)
    return false;
...
function deleteTeacher(teacherName) {
  if (
    !confirm(`Tem certeza que deseja excluir o professor "${teacherName}"?`)
  ) {
    return false;
  }
        ...
        allData[date][shift].forEach((record) => {
          if (!record.id || !seenIds.has(record.id)) {
            if (!record.id) {
              ...
              console.log(
                " [CLEAN] ID criado para registro sem ID:",
                record.id,
              );
            }
...
function handleRoomRegistration() {
  ...
  if (sala && sala.length > 50) {
    showNotification(
      "Nome da sala não pode ter mais de 50 caracteres.",
      "warning",
    );
    return;
  }
  ...
  if (numero && numero.length > 10) {
    showNotification(
      "Número da sala não pode ter mais de 10 caracteres.",
      "warning",
    );
    return;
  }
  ...
  const roomExists = registredRooms.some((room) => {
    return (
      ...
      room.bloco.toLowerCase().trim() ===
        "bloco " + bloco.toLowerCase().trim() &&
...
function loadRoomsTable() {
  ...
  document.getElementById("total-registered-rooms").innerHTML =
    dropdownData.length;
...
function handleKeyAction(recordId, currentStatus) {
  ...
  if (!record) {
    console.log(
      " [DEBUG] Não encontrado no turno atual, procurando globalmente...",
    );
    // Garantir que dataByDateAndShift está atualizada
    dataByDateAndShift = JSON.parse(
    ...
    for (const date in dataByDateAndShift) {
      for (const shift in dataByDateAndShift[date]) {
        ...
        if (Array.isArray(shiftData)) {
          const foundRecord = shiftData.find(
            (r) => String(r.id) === String(recordId),
          );
          if (foundRecord) {
            console.log(
              " [DEBUG] Registro encontrado em",
              date,
              shift,
              ":",
              foundRecord,
  ...
  if (currentStatus === "em_uso") {
    ...
    showNotification(
      `Chave devolvida por ${record.professorName} às ${record.returnTime}`,
      "success",
    );
  } else if (
    ...
    // Mostrar notificação Bootstrap
    showNotification(
      `Chave retirada por ${record.professorName} às ${record.withdrawalTime}`,
      "info",
    );
  }
  ...
  if (typeof saveDataToFirebase === "function") {
    ...
    if (targetData && Array.isArray(targetData) && targetData.length > 0) {
      saveDataToFirebase(saveDate, saveShift, targetData)
        .then(() => {
...
function initializePainelAdm() {
  ...
  setTimeout(() => {
    ...
    if (tabsElement) {
      console.log(
        "Conteúdo das abas após renderização:",
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const editButton = document.querySelector(
    'button[title="Editar Configurações"]',
  );
  if (editButton) {
...
function resetAllDropdowns() {
  ...
  document.getElementById("manualValueRoomNumber").innerText =
    "Selecione o número da sala";

  // Reseta o gradiente do dropdown selecionado
  const blockSelected = document.querySelector(
    "#manual-block-dropdown .selected",
  );
  const roomSelected = document.querySelector(
    "#manual-room-dropdown .selected",
  );
  const roomNumberSelected = document.querySelector(
    "#manual-room-number-dropdown .selected",
  );
  ...
  const roomNumberDropdown = document.getElementById(
    "manual-room-number-dropdown",
  );
  ...
  const blockOptions = document.querySelector(
    "#manual-block-dropdown .options",
  );
  const roomOptions = document.querySelector("#manual-room-dropdown .options");
  const roomNumberOptions = document.querySelector(
    "#room-number-dropdown .options",
  );
...
function getUniqueRoomNumbersForRoom(rooms, selectedBlock, selectedRoom) {
  const roomNumbers = rooms
    .filter(
      (room) => room.bloco === selectedBlock && room.sala === selectedRoom,
    )
...
function populateManualBlockDropdown() {
  ...
  blockOptions.querySelectorAll(".option").forEach((option) => {
    option.addEventListener("click", function (e) {
      ...
      document
        .querySelector("#manual-block-dropdown .selected")
        .classList.remove("active");
      document
        .querySelector("#manual-block-dropdown .selected")
        .classList.add("gradient");
      document
        .querySelector("#manual-block-dropdown .options")
        .classList.remove("show");
      document
        .querySelector("#manual-block-dropdown")
        .classList.add("selectedOption");

      // Mostra o dropdown de salas e popula
      document
        .getElementById("manual-room-dropdown")
        .classList.remove("hidden");
      populateManualRoomDropdown(selectedBlock);

      // Esconde o dropdown de números de sala
      document
        .getElementById("manual-room-number-dropdown")
        .classList.add("invisible");
      document.getElementById("manualValueRoomNumber").textContent =
        "Selecione o número da sala";
    });
  });
}
...
function populateManualRoomDropdown(selectedBlock) {
  ...
  roomOptions.innerHTML = roomsInBlock
    .map(
      (room) => `
  ...
  roomOptions.querySelectorAll(".option").forEach((option) => {
    option.addEventListener("click", function (e) {
      ...
      if (selectedRoom !== manualCurrentSelections.room) {
        ...
        document
          .getElementById("manual-room-number-dropdown")
          .classList.remove("noOptions");
        document
          .getElementById("manual-selected-room-number")
          .classList.remove("gradient");
      }
      ...
      document
        .querySelector("#manual-room-dropdown .selected")
        .classList.remove("active");
      document
        .querySelector("#manual-room-dropdown .selected")
        .classList.add("gradient");
      document
        .querySelector("#manual-room-dropdown .options")
        .classList.remove("show");
      document
        .querySelector("#manual-room-dropdown")
        .classList.add("selectedOption");
...
function populateManualRoomNumberDropdown(selectedBlock, selectedRoom) {
  ...
  const rooms = getDropdownData();
  const roomNumbers = getUniqueRoomNumbersForRoom(
    rooms,
    selectedBlock,
    selectedRoom,
  );
  ...
  roomNumberOptions.innerHTML = roomNumbers
    .map(
      (number) => `
    ...
    .forEach((option) => {
      option.addEventListener("click", function (e) {
        ...
        document.getElementById("manualValueRoomNumber").textContent =
          selectedRoomNumber;
        document
          .querySelector("#manual-room-number-dropdown .options")
          .classList.remove("show");
        document
          .querySelector("#manual-room-number-dropdown .selected")
          .classList.add("gradient");
        document
          .querySelector("#manual-room-number-dropdown .selected")
          .classList.remove("active");
...
function loadManualAllocationsTable() {
  ...
  try {
    ...
    const allDateShiftData = JSON.parse(
      localStorage.getItem("allDateShiftData") || "{}",
    );
      ...
      Object.keys(allDateShiftData[date]).forEach((shift) => {
        ...
        if (Array.isArray(shiftData)) {
          ...
          Object.keys(shiftData).forEach((recordKey) => {
            ...
            if (record && record.tipo === "manual_allocation") {
              console.log(
                " [DEBUG] Alocação manual encontrada (objeto):",
                record,
              );
              manualAllocations.push(record);
            }
    ...
    console.log(
      " [DEBUG] Total de alocações manuais encontradas:",
      manualAllocations.length,
    );
    ...
    tableBody.innerHTML = manualAllocations
      .map((allocation) => {
...
function deleteManualAllocation(allocationId, dataAlocacao, periodo) {
  ...
  try {
    ...
    console.log(
      " [DEBUG] Dados antes da exclusão:",
      dataByDateAndShift[dataAlocacao]?.[periodo],
    );
    ...
    if (
      ...
      if (Array.isArray(shiftData)) {
        ...
        dataByDateAndShift[dataAlocacao][periodo] = shiftData.filter(
      ...
      if (typeof saveDataToFirebase === "function") {
        ...
        if (
          !Array.isArray(updatedShiftData) &&
          typeof updatedShiftData === "object"
        ) {
          dataToSync = Object.values(updatedShiftData).filter(
            (item) => item != null,
          );
        }

        saveDataToFirebase(dataAlocacao, periodo, dataToSync)
          .then(() => {
            console.log(
              " [ALOCAÇÃO MANUAL] Exclusão sincronizada com Firebase com sucesso!",
            );
          })
          .catch((error) => {
            console.error(
              " [ALOCAÇÃO MANUAL] Erro ao sincronizar exclusão no Firebase:",
              error,
            );
            showNotification(
              "Alocação excluída localmente, mas houve erro na sincronização com o servidor.",
              "warning",
            );
          });
      } else {
        console.warn(
          " [ALOCAÇÃO MANUAL] Função saveDataToFirebase não disponível - exclusão apenas local",
        );
      }
...
function getAvailableShiftsText(currentShift) {
  ...
  const availableShifts = allShifts.filter(
    (shift) => shiftOrder[shift] >= currentOrder,
  );
...
function setupManualDropdownInteractions() {
  // Dropdown de blocos
  const blockDropdown = document.querySelector(
    "#manual-block-dropdown .selected",
  );
  const blockOptions = document.querySelector(
    "#manual-block-dropdown .options",
  ...
  const roomNumberDropdown = document.querySelector(
    "#manual-room-number-dropdown .selected",
  );
  const roomNumberOptions = document.querySelector(
    "#manual-room-number-dropdown .options",
  );
  ...
  document.addEventListener("click", function () {
    document
      .querySelectorAll("#manualAllocationModal .selected")
      .forEach((dropdown) => {
        dropdown.classList.remove("active");
      });
    document
      .querySelectorAll("#manualAllocationModal .options")
      .forEach((options) => {
        options.classList.remove("show");
      });
  });
...
async function handleManualAllocation() {
  ...
  console.log(
    "� [DEBUG] Comparação selectedDate < today:",
    selectedDate < today,
  );
  ...
  if (selectedDate.getTime() === today.getTime()) {
    ...
    if (selectedShiftOrder < currentShiftOrder) {
      showNotification(
        `Para hoje, você só pode alocar para o turno atual (${currentShift}) ou turnos posteriores. Turnos disponíveis: ${getAvailableShiftsText(currentShift)}.`,
        "warning",
      );
      return;
    }
  ...
  if (!turnosValidos.includes(turno)) {
    showNotification(
      "Turno inválido. Selecione Manhã, Tarde ou Noite.",
      "warning",
    );
    return;
  }
  ...
  if (professor && professor.length > 100) {
    showNotification(
      "Nome do professor não pode ter mais de 100 caracteres.",
      "warning",
    );
    return;
  }
  ...
  if (curso && curso.length > 100) {
    showNotification(
      "Nome do curso não pode ter mais de 100 caracteres.",
      "warning",
    );
    return;
  }
  ...
  if (observacoes && observacoes.length > 200) {
    showNotification(
      "Observações não podem ter mais de 200 caracteres.",
      "warning",
    );
    return;
  }
  ...
  console.log(
    " [DEBUG] Alocação manual criada com ID:",
    manualAllocation.id,
    "para sala:",
    manualAllocation.sala,
  );
  ...
  try {
    ...
    console.log(
      " [ALOCAÇÃO MANUAL] Verificando dados existentes no Firebase...",
    );

    // Verificar se já existem dados no localStorage para esta data/turno
    const hasLocalData =
      dataByDateAndShift[dataAlocacao][turno] &&
      Array.isArray(dataByDateAndShift[dataAlocacao][turno]) &&
      dataByDateAndShift[dataAlocacao][turno].length > 0;

    if (hasLocalData) {
      console.log(
        ` [ALOCAÇÃO MANUAL] Dados locais encontrados (${dataByDateAndShift[dataAlocacao][turno].length} registros). Adicionando nova alocação...`,
      );
      // Se já temos dados locais, assumimos que estão sincronizados
      dataByDateAndShift[dataAlocacao][turno].push(manualAllocation);
    } else {
      console.log(
        " [ALOCAÇÃO MANUAL] Sem dados locais. Buscando dados do Firebase...",
      );
      ...
      if (typeof loadDataFromFirebase === "function") {
        try {
          ...
          console.log(
            ` [ALOCAÇÃO MANUAL] Dados do Firebase carregados: ${firebaseData ? firebaseData.length : 0} registros`,
          );
          ...
          if (
            ...
            console.log(
              ` [ALOCAÇÃO MANUAL] Usando ${firebaseData.length} registros existentes do Firebase`,
            );
            ...
            console.log(
              " [ALOCAÇÃO MANUAL] Firebase sem dados. Criando novo array com a alocação.",
            );
            dataByDateAndShift[dataAlocacao][turno] = [manualAllocation];
          }
        } catch (error) {
          console.warn(
            " [ALOCAÇÃO MANUAL] Erro ao buscar dados do Firebase:",
            error,
          );
        ...
        console.log(
          " [ALOCAÇÃO MANUAL] loadDataFromFirebase não disponível. Inicializando array com a nova alocação...",
        );
        dataByDateAndShift[dataAlocacao][turno] = [manualAllocation];
      }
    }
    ...
    console.log(
      "   - Total de registros no turno:",
      dataByDateAndShift[dataAlocacao][turno].length,
    );
    ...
    if (typeof saveDataToFirebase === "function") {
      ...
      saveDataToFirebase(
        ...
        .then(() => {
          console.log(
            " [ALOCAÇÃO MANUAL]: Dados salvos no Firebase com sucesso!",
          );
        })
        .catch((error) => {
          console.error(
            " [ALOCAÇÃO MANUAL]: Erro ao salvar no Firebase:",
            error,
          );
        });
    }
    ...
    window.dispatchEvent(
      new CustomEvent("shiftDataUpdated", {
        detail: {
          shift: turno,
    ...
    showNotification(
      `Alocação manual registrada com sucesso para ${formatDate(dataAlocacao)} - ${capitalizeFirst(turno)}!`,
      "success",
    );
    ...
    if (dataAlocacao === selectedDate) {
      console.log(
        " [ALOCAÇÃO MANUAL] Atualizando tabela pois a alocação foi feita para a data atual",
      );
      renderTable();
    } else {
      console.log(
        "� [ALOCAÇÃO MANUAL] Alocação feita para data diferente da atual. Visualize a data",
        dataAlocacao,
        "para ver a alocação",
      );
    }
    ...
    showNotification(
      "Erro ao salvar alocação manual. Tente novamente.",
      "error",
    );
  }
}
